// list.dart
import 'package:gkvk/models/data_model.dart';

List<Question> questionsPage1 = [
  Question(
    questionText: '1. LRI based fertilizer recommendation to crops is a boon to farmers',
    options: ['Strongly agree', 'Agree', 'Neutral', 'Disagree', 'Strongly disagree'],
  ),
  Question(
    questionText: '2. Do not see much advantage in LRI based fertilizer application for fertilizer intensive crops',
    options: ['Strongly agree', 'Agree', 'Neutral', 'Disagree', 'Strongly disagree'],
  ),
  Question(
    questionText: '3. LRI based fertilizer application help to optimize crop yields',
    options: ['Strongly agree', 'Agree', 'Neutral', 'Disagree', 'Strongly disagree'],
  ),
  Question(
    questionText: '4. LRI based fertilizer application is suitable for dryland crops only ',
    options: ['Strongly agree', 'Agree', 'Neutral', 'Disagree', 'Strongly disagree'],
  ),
  Question(
    questionText: '5. LRI based fertilizer application do not harm useful microorganisms present in the soil',
    options: ['Strongly agree', 'Agree', 'Neutral', 'Disagree', 'Strongly disagree'],
  ),
  Question(
    questionText: '6. The RSK staff are competent to guide on fertilizers dosage for crops based on LRI card',
    options: ['Strongly agree', 'Agree', 'Neutral', 'Disagree', 'Strongly disagree'],
  ),
  Question(
    questionText: '7. The information presented in LRI card is enough to take rational decisions on nutrients’ selection',
    options: ['Strongly agree', 'Agree', 'Neutral', 'Disagree', 'Strongly disagree'],
  ),
  Question(
    questionText: '8. The accuracy of information presented in LRI card is doubtful',
    options: ['Strongly agree', 'Agree', 'Neutral', 'Disagree', 'Strongly disagree'],
  ),
  Question(
    questionText: '9. Not sure of getting better economic gains by adopting LRI based fertilizer application',
    options: ['Strongly agree', 'Agree', 'Neutral', 'Disagree', 'Strongly disagree'],
  ),
  Question(
    questionText: '10. LRI based approach help to save cost of fertilizers compare to indiscriminate use',
    options: ['Strongly agree', 'Agree', 'Neutral', 'Disagree', 'Strongly disagree'],
  ),
  Question(
    questionText: '11. Local Fertilizer Dealers’ advice on fertilizer dosage is more dependable as they know soil characteristics and crop needs of the locality',
    options: ['Strongly agree', 'Agree', 'Neutral', 'Disagree', 'Strongly disagree'],
  ),
  // Add more questions as needed
];
List<Question> questionsPage2 = [
  Question(
    questionText: '1. Format or layout of presentation of information in LRI card',
    options: ['Very Much', 'Much', 'Not'],
  ),
  Question(
    questionText: '2. Adequacy of information presented in LRI card ',
    options: ['Very Much', 'Much', 'Not'],
  ),
  Question(
    questionText: '3. Usefulness of information presented in LRI card',
    options: ['Very Much', 'Much', 'Not'],
  ),
  Question(
    questionText: '4. Easiness to understand the contents of LRI card for its application',
    options: ['Very Much', 'Much', 'Not'],
  ),
  Question(
    questionText: '5. Adopt LRI based fertilizer application to crops grown by me',
    options: ['Very Much', 'Much', 'Not'],
  ),
];
List<Question> questionsPage3 = [
  Question(
    questionText: '1. Whose advice do you seek to decide the type and quantity of fertilizers?',
    options: ['On my own', 'Fertilizer dealer', 'RSK staff', 'Neighbors'],
  ),
  Question(
    questionText: '2. What is the most appropriate information source to decide required quantities of fertilizers for the crops?',
    options: ['Own experience', 'Package of practices', 'Soil test report', 'LRI card'],
  ),
  Question(
    questionText: '3. Have you applied fertilizers as per LRI card?',
    options: ['No', 'Not aware', 'Not skillful to use LRI information', 'Yes'],
  ),
  Question(
    questionText: '4. If you have applied fertilizers as per LRI card, what is your opinion?',
    options: [
      'Cannot differentiate the benefits',
      'Able to save fertilizer cost',
      'Able to get more yield',
      'b and c'
    ],
  ),
  Question(
    questionText: '5. If you are trained on the use of LRI cards, will you follow LRI based fertilizer application?',
    options: [
      'No',
      'Try next season/year',
      'Try this season in smaller area',
      'Try this season for all the crops'
    ],
  ),
];
List<Question> questionsPage4 = [
  Question(
    questionText: '1. Under which project, the LRI card is distributed to farmers?',
    options: ['Krishi Bhagya', 'Precision farming', 'Sujala/ REWARD', 'National Horticulture Mission'],
  ),
  Question(
    questionText: '2. What information can a farmer get from an LRI card?',
    options: ['Suitable conservation measures', 'Soil fertility status', 'Crop suitability', 'All the above'],
  ),
  Question(
    questionText: '3. What is the inference for yellow color in respect of soil nutrient status in LRI cards?',
    options: ['Very high', 'High', 'Medium', 'Low'],
  ),
  Question(
    questionText: '4. What is the adjustment factor to align fertilizers when soil nutrient status is low?',
    options: ['RDF x 1.67', 'RDF x 1.33', 'RDF x 1.00', 'RDF x 0.66'],
  ),
  Question(
    questionText: '5. RDF for maize is 60:30:15 kg/ acre. If soil nitrogen content is medium, what is the adjustment factor to be followed?',
    options: ['RDF x 1.00', 'RDF x 0.66', 'RDF x 1.33', 'RDF x 1.67'],
  ),
  Question(
    questionText: '6. What is the role of nitrogen in plants?',
    options: ['For better growth', 'For disease resistance', 'For stopping fruit drop', 'For pest resistance'],
  ),
  Question(
    questionText: '7. What are the ill effects of using excess chemical fertilizers?',
    options: ['Reduction in the soil health', 'Contamination of water bodies', 'Harm soil microorganisms', 'All the above'],
  ),
  Question(
    questionText: '8. If your plants suffer from nitrogen deficiency, which part of young leaves will turn yellow first?',
    options: ['Lower part', 'Upper part', 'Fruits', 'Tip of the plant'],
  ),
  Question(
    questionText: '9. If soil is characterized as very acidic, what can be done to reduce the harmful effects associated with this?',
    options: ['Apply gypsum', 'Apply lime', 'Apply manure', 'Apply urea'],
  ),
  Question(
    questionText: '10. To apply 46 kg of nitrogen, how much urea to be used?',
    options: ['50 kg', '100 kg', '150 kg', '200 kg'],
  ),
  Question(
    questionText: '11. Which of the following is a secondary nutrient?',
    options: ['Potash', 'Phosphorous', 'Sulphur', 'Nitrogen'],
  ),
  Question(
    questionText: '12. What type of data from an LRI is most important for determining fertilizer needs?',
    options: ['Soil depth', 'Soil slope', 'Soil fertility', 'Weather patterns'],
  ),
  Question(
    questionText: '13. While interpreting LRI data for fertilizer application, what should a farmer consider?',
    options: ['Soil fertility status', 'Crop nutrient needs', 'Crop season', 'All the above'],
  ),
  Question(
    questionText: '14. To apply 46 kg of phosphorus, how much DAP to be used?',
    options: ['46 kg', '100 kg', '150 kg', '200 kg'],
  ),
  Question(
    questionText: '15. What are the benefits of LRI based fertilizer application?',
    options: ['Optimized yield', 'Balance in soil fertility', 'Better economic returns', 'All the above'],
  ),
];
List<Question> questionsPage5 = [
  Question(
    questionText: '1. What is the main purpose of a land resource inventory (LRI) for fertilizer application?',
    options: [
      'To increase overall fertilizer use',
      'To create a specific fertilizer application plan for each field area',
      'To eliminate the need for soil testing',
      'To simplify the fertilizer application process'
    ],
  ),
  Question(
    questionText: '2. What type of data from an LRI is most important for determining fertilizer needs?',
    options: [
      'Land area',
      'Crop type history',
      'Soil fertility',
      'Weather patterns'
    ],
  ),
  Question(
    questionText: '3. A farmer notices a specific area of their field with lower soil fertility in their LRI data. What is the most appropriate action based on this information?',
    options: [
      'Apply the same amount of fertilizer across the entire field',
      'Increase the fertilizer application rate only in that specific area',
      'Plant a different type of crop in that area',
      'Ignore this information as it\'s likely not significant'
    ],
  ),
  Question(
    questionText: '4. LRI data is most valuable for fertilizer application in:',
    options: [
      'Large, uniform fields',
      'Diverse fields with varying soil conditions and crop types',
      'Fields with a long history of organic fertilizer use',
      'Fields located near major waterways'
    ],
  ),
  Question(
    questionText: '5. What is the main difference between blanket application and LRI based application of fertilizer?',
    options: [
      'Blanket applies more fertilizer overall',
      'LRI applies fertilizer based on specific needs of different areas',
      'Broadcast application requires specialized equipment',
      'LRI based application is faster than blanket application'
    ],
  ),
  Question(
    questionText: '6. While interpreting LRI data for fertilizer application, a farmer should consider:',
    options: [
      'Only the current soil fertility levels',
      'Both soil fertility and crop nutrient needs',
      'Solely the historical fertilizer application rates',
      'Primarily the weather forecast for the upcoming season'
    ],
  ),
  Question(
    questionText: '7. Which of the following is NOT a common method for collecting LRI data for fertilizer application?',
    options: [
      'Interviewing the landowner about past land use practices',
      'Using remote sensing imagery to analyze field characteristics',
      'Taking soil samples from different locations within the field',
      'Observing vegetation health and growth patterns across the field'
    ],
  ),
  Question(
    questionText: '8. What is the biggest challenge a farmer might face when implementing LRI-based fertilizer application?',
    options: [
      'The cost of soil testing',
      'Difficulty in interpreting LRI data',
      'The physical effort required for data collection',
      'The lack of available fertilizer types'
    ],
  ),
  Question(
    questionText: '9. What is the most important factor for a farmer to consider when choosing a fertilizer based on LRI data?',
    options: [
      'The brand name of the fertilizer',
      'The specific nutrient content of the fertilizer to address soil deficiencies',
      'The color and texture of the fertilizer product',
      'The marketing claims made about the fertilizer'
    ],
  ),
  Question(
    questionText: '10. While interpreting LRI card, what does red color indicate with respect to application of fertilizers?',
    options: [
      'Increased application of fertilizers',
      'Decreased application of fertilizers',
      'No change in application rates',
      'Switching to a completely different fertilizer type'
    ],
  ),
  Question(
    questionText: '11. What information can a farmer get from LRI card?',
    options: [
      'Suitable conservation measures',
      'Soil fertility status',
      'Crop suitability',
      'All the above'
    ],
  ),
  Question(
    questionText: '12. What is the adjustment factor to align fertilizers when soil nutrient status is low?',
    options: [
      'RDF x 1.67',
      'RDF x 1.33',
      'RDF x 1.00',
      'RDF x 0.66'
    ],
  ),
  Question(
    questionText: '13. RDF for maize is 60:30:15 kg/ acre. If soil nitrogen content is medium, what is the adjustment factor to be followed?',
    options: [
      'RDF x 1.00',
      'RDF x 0.66',
      'RDF x 1.33',
      'RDF x 1.67'
    ],
  ),
  Question(
    questionText: '14. What are the ill effects of using excess chemical fertilizers?',
    options: [
      'Reduction in the soil health',
      'Contamination of water bodies',
      'Harm soil microorganisms',
      'All the above'
    ],
  ),
  Question(
    questionText: '15. Which of the following is a secondary nutrient?',
    options: [
      'Potash',
      'Phosphorous',
      'Sulphur',
      'Nitrogen'
    ],
  ),
];
List<Question> statementAcceptance = [
  Question(
    questionText: '1. Format or layout of presentation of information in LRI card',
    options: ['Very Much', 'Much', 'Not'],
  ),
  Question(
    questionText: '2. Adequacy of information presented in LRI card',
    options: ['Very Much', 'Much', 'Not'],
  ),
  Question(
    questionText: '3. Usefulness of information presented in LRI card',
    options: ['Very Much', 'Much', 'Not'],
  ),
  Question(
    questionText: '4. Easiness to understand the contents of LRI card for its application',
    options: ['Very Much', 'Much', 'Not'],
  ),
  Question(
    questionText: '5. Promote LRI based fertilizer application to crops grown by farmers',
    options: ['Very Much', 'Much', 'Not'],
  ),
];
