...
class _FarmersTabState extends State<FarmersTab> {
  ...
  Future<void> uploadFarmerData(int aadharNumber) async {
    ...
    try {
      ...
      if (watershedId != null) {
        watershedData = await waterShedDB.read(watershedId);
      }

      final firestore = FirebaseFirestore.instance;
      final batch = firestore.batch();

      // Reference to farmer collection
      final farmerRef = firestore.collection('farmers').doc(aadharNumber.toString());

      // Upload image to Firebase Storage with retries and error handling
          ...
          try {
            ...
            while (retryCount > 0) {
              try {
                ...
                retryCount--;
                print('Upload failed. Retries left: $retryCount. Error: $e');
                if (retryCount == 0) {
      ...
      // Set farmer data in batch
      batch.set(farmerRef, mutableFarmerData);

      // Set survey data in batch if not null
      if (surveyData1 != null) {
      ...
      // Set crop details in batch if not empty
      ...
      // Set watershed data in batch if not null
      ...
      // Delete records from local database after upload
  ...
  Future<void> uploadAllData() async {
    ...
    final dealers = await fetchAlldealers();

    if (remainingFarmers.isEmpty && dealers.isEmpty ) {
      final waterShedDB = WaterShedDB();
...
class _DealersTabState extends State<DealersTab> {
  ...
  Future<void> uploadDealerData(int aadharNumber) async {
    ...
    try {
      ...
      // Reference to dealer collection
      ...
      if (imagePath != null && imagePath.isNotEmpty) {
        ...
        if (file.existsSync()) {
          try {
            final storageRef = FirebaseStorage.instance.ref().child('dealer_images/$aadharNumber.jpg');
            int retryCount = 3;
            String? downloadUrl;
      ...
      // Add watershed data to batch if available
      if (watershedData != null) {
  ...
  Future<void> uploadAllData() async {
    ...
    final remainingDealers = await fetchAllDealers();
    final Farmers = await fetchAllfarmers();

    if (remainingDealers.isEmpty && Farmers.isEmpty) {
      final waterShedDB = WaterShedDB();
      await waterShedDB.deleteAll();
    }
    setState(() {
